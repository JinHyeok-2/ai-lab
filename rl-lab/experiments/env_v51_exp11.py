#!/usr/bin/env python3
# ETH 선물거래 RL 환경 v5.1-exp11
# LSTM + gamma 0.99 + DD 패널티 완화 (exp09의 극보수 문제 해결)
#
# exp02 대비 변경:
#   1. DD 패널티 완화: 5%/-0.05 → 8%/-0.03, 10%/-0.15 → 15%/-0.08
#   2. max_drawdown: 15% → 20% (LSTM에게 더 넓은 탐색 공간)
#   3. 쿨다운 패널티 완화: 0.008 → 0.004
...
class ETHTradingEnvV51Exp11(gym.Env):
    """v5.1-exp11: LSTM용 DD 패널티 완화"""
    ...
    def __init__(self, df: pd.DataFrame,
                 ...
                 max_drawdown: float = 0.20,    # 15% → 20% (완화)
    ...
    def _find_bear_segments(self):
        if len(self.df) < 300: self.bear_starts = []; return
        returns = self.df["close"].pct_change(200).fillna(0)
        bear_mask = returns < -0.03
        max_start = len(self.df) - self.max_episode_len - 1
        self.bear_starts = [i for i in bear_mask[bear_mask].index.tolist()
                            if self.window_size <= i <= max_start]
        if len(self.bear_starts) < 10: self.bear_starts = []

    def _get_trend(self):
        i = self.current_step
        if i < self.window_size: return 0
        row = self.processed.iloc[i]
        score = float(row.get("ema_ratio", 0)) * 10 + float(row.get("price_chg_1h", 0)) * 3
        if float(row.get("adx_norm", 0)) > 0.25: score *= 1.5
    ...
    def _get_adx(self):
        i = self.current_step
        if i >= len(self.processed): return 0.0
    ...
    def reset(self, seed=None, options=None):
        ...
        if max_start <= self.window_size:
            start = self.window_size; self.episode_end = len(self.df) - 1
        else:
            if self.bear_ratio > 0 and self.bear_starts and self.np_random.random() < self.bear_ratio:
                start = int(self.np_random.choice(self.bear_starts))
            else: start = int(self.np_random.integers(self.window_size, max_start + 1))
            self.episode_end = min(start + self.max_episode_len, len(self.df) - 1)
        self.current_step = start; self.episode_start = start
        self.balance = self.initial_balance; self.peak_balance = self.initial_balance
        self.position = 0; self.entry_price = 0.0; self.hold_steps = 0; self.cooldown = 0
        self.total_trades = 0; self.win_trades = 0; self.total_pnl = 0.0; self.trade_history = []
    ...
    def _get_obs(self):
        ...
        for i in range(self.current_step - self.window_size, self.current_step):
            ...
            feat += [float(self.position), float(np.clip(upnl, -1, 1)),
                     float(min(self.hold_steps, 50) / 50), float(min(self.cooldown, 10) / 10)]
            rows.append(feat)
    ...
    def step(self, action: int):
        ...
        reward = 0.0
        trend = self._get_trend(); adx = self._get_adx()
        ...
        def close_pos():
            ...
            reward += trade_pnl * 15
            if trade_pnl > 0: self.win_trades += 1; reward += 0.01
            else: reward -= 0.008
            self.total_trades += 1; self.total_pnl += trade_pnl
            self.trade_history.append({"step": self.current_step, "pnl": trade_pnl,
                                        "hold_steps": self.hold_steps, "price": price, "position": self.position})
            self.position = 0; self.entry_price = 0.0; self.hold_steps = 0
            self.cooldown = self.cooldown_steps

        TREND_BONUS = 0.006; TREND_PENALTY = 0.006
        adx_gate = adx > 0.25

        if action == 1:
            if self.position == -1: close_pos()
            if self.position == 0:
                if self.cooldown > 0: reward -= 0.004 * self.cooldown  # 완화: 0.008 → 0.004
                elif adx_gate:
                    if trend == 1: reward += TREND_BONUS
                    elif trend == -1: reward -= TREND_PENALTY
                self.balance *= (1 - self.fee_rate); self.position = 1
                self.entry_price = price; self.hold_steps = 0
            else: reward -= 0.005
        elif action == 2:
            if self.position == 1: close_pos()
            if self.position == 0:
                if self.cooldown > 0: reward -= 0.004 * self.cooldown
                elif adx_gate:
                    if trend == -1: reward += TREND_BONUS
                    elif trend == 1: reward -= TREND_PENALTY
                self.balance *= (1 - self.fee_rate); self.position = -1
            ...
            if self.position != 0: close_pos()
            else: reward -= 0.005
        else:
            if self.position != 0:
                self.hold_steps += 1
                if self.position * trend > 0: reward += 0.002
                elif self.position * trend < 0: reward -= 0.003
                if self.hold_steps > 150: reward -= 0.002

        # ── DD 패널티 완화 (LSTM이 공포 학습하지 않도록) ──
        self.peak_balance = max(self.peak_balance, self.balance)
        drawdown = (self.peak_balance - self.balance) / self.peak_balance

        if drawdown > 0.15:
            reward -= 0.08   # DD 15% 이상 (exp02: 10%/-0.15)
        elif drawdown > 0.08:
            reward -= 0.03   # DD 8% 이상 (exp02: 5%/-0.05)

        if self.balance < self.initial_balance * 0.3: reward -= 0.3

        self.current_step += 1
        terminated = self.current_step >= self.episode_end
        truncated = (self.balance < self.initial_balance * 0.05 or drawdown > self.max_drawdown)
        if truncated: reward -= 0.5
    ...
    def render(self):
        ...
        print(f"Step {self.current_step:5d} | ${price:,.2f} | 잔고 ${self.balance:,.0f} | {pos_str} | 거래 {self.total_trades}회 | DD {dd:.1f}%")
