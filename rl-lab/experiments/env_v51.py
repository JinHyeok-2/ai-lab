#!/usr/bin/env python3
# ETH 선물거래 RL 환경 v5.1-exp01
# v5 대비 변경: 보상 대칭화 (숏 편향 해소)
#
# 변경 사항:
#   1. 추세 순행/역행 보상 대칭화: 롱/숏 동일 (+0.012 / -0.012)
#   2. 커리큘럼 하락장 초기 비율 70% → 50%
#   3. PnL 보상 스케일 20 유지 (MDD 개선은 exp02에서)
...
class ETHTradingEnvV51(gym.Env):
    """v5.1-exp01: 보상 대칭화 환경"""
    ...
    def step(self, action: int):
        ...
        # ── [핵심 변경] 대칭 보상 함수 ────────────────────
        # 추세 순행 진입: +0.012 (롱이든 숏이든)
        # 추세 역행 진입: -0.012 (롱이든 숏이든)
        TREND_FOLLOW_BONUS  = 0.012
        TREND_AGAINST_PENALTY = 0.012
        ...
        if action == 1:   # 롱 진입
            ...
            if self.position == 0:
                ...
                # [변경] 대칭 보상
                if trend == 1:
                    reward += TREND_FOLLOW_BONUS    # 상승추세 롱 = 순행
                elif trend == -1:
                    reward -= TREND_AGAINST_PENALTY  # 하락추세 롱 = 역행
            ...
            if self.position == 0:
                if self.cooldown > 0:
                    reward -= 0.005 * self.cooldown
                # [변경] 대칭 보상 (v5는 숏 +0.02 / 롱 +0.008 비대칭)
                if trend == -1:
                    reward += TREND_FOLLOW_BONUS    # 하락추세 숏 = 순행
                elif trend == 1:
                    reward -= TREND_AGAINST_PENALTY  # 상승추세 숏 = 역행
                self.balance    *= (1 - self.fee_rate)
                self.position    = -1
                self.entry_price = price
                self.hold_steps  = 0
            ...
            if self.position != 0:
                self.hold_steps += 1
                # [변경] 관망 보상도 대칭화
                if self.position * trend > 0:  # 포지션과 추세 같은 방향
                    reward += 0.002
                elif self.position * trend < 0:  # 포지션과 추세 반대
                    reward -= 0.003
