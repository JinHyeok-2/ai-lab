#!/usr/bin/env python3
# ETH 선물거래 RL 환경 v5.1-exp02
# exp01(보상 대칭화) + 과다매매 억제 + DD 단계 패널티 + ADX 필터
#
# exp01 대비 변경:
#   1. 추세 진입 보상 축소: 0.012 → 0.006 (무분별 진입 억제)
#   2. ADX 필터: ADX > 0.25일 때만 추세 보상 적용
#   3. cooldown 5 → 8 (거래 간격 강제)
#   4. DD 단계별 패널티: 5% → -0.05, 10% → -0.15, 15% → truncate
#   5. 거래당 PnL 보상 스케일: 20 → 15 (안정성 우선)
...
class ETHTradingEnvV51Exp02(gym.Env):
    """v5.1-exp02: 대칭 보상 + 과다매매 억제 + DD 관리"""
    ...
    def step(self, action: int):
        ...
        def close_pos():
            ...
            self.entry_price = 0.0
            self.hold_steps  = 0
            self.cooldown    = self.cooldown_steps  # [변경3] 8캔들

        # ── 행동 처리 ────────────────────────────────────
        # [변경1] 진입 보상 축소: 0.012 → 0.006
        # [변경2] ADX 필터: ADX > 0.25일 때만 추세 보상
        TREND_BONUS   = 0.006
        TREND_PENALTY = 0.006
        adx_gate = adx > 0.25  # ADX 필터
        ...
        if action == 1:   # 롱 진입
            ...
            if self.position == 0:
                if self.cooldown > 0:
                    reward -= 0.008 * self.cooldown  # 쿨다운 패널티 강화
                elif adx_gate:  # ADX 충분할 때만 추세 보상
                    if trend == 1:
                        reward += TREND_BONUS
                    elif trend == -1:
