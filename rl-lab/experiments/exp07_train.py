#!/usr/bin/env python3
# exp07: N_ENVS 8 + batch 512 (exp02 환경 동일)
# 더 다양한 경험 수집으로 안정적 학습
...
class CurriculumCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        log.info(f"[{self.start_time.strftime('%H:%M:%S')}] {EXP_NAME} 학습 시작 ({self.total_steps:,} 스텝, {N_ENVS} envs)")
...
def train():
    ...
    model = PPO(
        ...
        n_steps=2048, batch_size=512, n_epochs=10,  # batch 256 → 512
    ...
    meta = {
        "experiment": EXP_NAME,
        "description": "N_ENVS 8 + batch 512 (exp02 환경 동일)",
        "changes": ["N_ENVS: 4 → 8", "batch_size: 256 → 512"],
...
def _plot(test_df, results, bh_ret):
    ...
    fig.suptitle(f"{EXP_NAME} (8 envs, batch 512) vs exp02 vs v5", fontsize=13, fontweight="bold")
