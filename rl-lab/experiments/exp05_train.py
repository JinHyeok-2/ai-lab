#!/usr/bin/env python3
# exp05: LSTM 정책 (exp02 환경 동일)
# MlpPolicy → RecurrentPPO + LstmPolicy
# 시계열 패턴 학습 강화로 숏 타이밍 개선 기대
...
def train():
    ...
    meta = {
        "experiment": EXP_NAME,
        "description": "LSTM 정책 (exp02 환경 동일)",
        "changes": ["MlpPolicy → MlpLstmPolicy (lstm_hidden=128, 1 layer)"],
...
def backtest(test_df):
    ...
    if exp02_path.exists():
        ...
        results["exp02"] = _run_backtest(env_exp02, exp02_path, use_lstm=False)
...
def _plot_comparison(test_df, results, bh_ret):
    ...
    fig.suptitle(f"{EXP_NAME} (LSTM) vs exp02 vs v5", fontsize=13, fontweight="bold")
