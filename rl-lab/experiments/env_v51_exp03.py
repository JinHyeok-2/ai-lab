#!/usr/bin/env python3
# ETH 선물거래 RL 환경 v5.1-exp03
# exp02 기반 + DD 패널티 완화 + 거래 빈도 증가 + 장기 관망 패널티
#
# exp02 대비 변경:
#   1. DD 패널티 완화: 5%/-0.05, 10%/-0.15 → 7%/-0.03, 12%/-0.10
#   2. PnL 보상 스케일: 15 → 18 (공격성 복원)
#   3. cooldown: 8 → 6
#   4. 장기 관망 패널티: 포지션 없이 50캔들 이상 → -0.001/step
#   5. 추세 진입 보상 소폭 상향: 0.006 → 0.008
...
class ETHTradingEnvV51Exp03(gym.Env):
    """v5.1-exp03: exp02 + DD완화 + 거래빈도 증가"""
    ...
    def reset(self, seed=None, options=None):
        ...
        self.hold_steps    = 0
        self.cooldown      = 0
        self.idle_steps    = 0   # [변경4] 포지션 없는 관망 카운터
        self.total_trades  = 0
        self.win_trades    = 0
        self.total_pnl     = 0.0
        self.trade_history = []
    ...
    def step(self, action: int):
        ...
        if drawdown > 0.12:
            reward -= 0.10   # DD 12% 이상 (exp02: 10%/-0.15)
        elif drawdown > 0.07:
            reward -= 0.03   # DD 7% 이상 (exp02: 5%/-0.05)
