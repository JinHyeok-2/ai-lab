#!/usr/bin/env python3
# ETH 선물거래 RL 환경 v5.1-long-only
# exp02 기반 + 행동공간 3개 (관망/롱/청산), 숏 완전 제거
# 숏 편향 근본 해결 — 실거래에서 숏 전패(-$8.64) 문제 대응
...
class ETHTradingEnvV51LongOnly(gym.Env):
    """v5.1-long-only: 롱 전용, 3행동 (관망/롱/청산)"""
    ...
    def step(self, action: int):
        ...
        if action == 1:   # 롱 진입
            if self.position == 0:
                if self.cooldown > 0:
                    ...
                    self.hold_steps  = 0
                    if trend == 1 and adx_gate:
                        reward += TREND_BONUS
                    elif trend == -1 and adx_gate:
        ...
        # action == 0: 관망 — 아무것도 안 함

        # 포지션 보유 중 미실현 PnL + 자동 손절/익절
        if self.position != 0:
            self.hold_steps += 1
            upnl = (price - self.entry_price) / self.entry_price * self.position * self.leverage

            # === 자동 손절: -5% 이하 강제 청산 ===
            if upnl < -0.05:
                close_pos()
                reward -= 0.03  # 손절 추가 페널티
            ...
            # === 자동 익절: +10% 이상 강제 청산 ===
                ...
                # 추세 정렬 관망 보상
                if trend == self.position and adx_gate:
                    reward += 0.002

                # === 손실 중 보유 패널티 (캔들 증가 시 강화) ===
                if upnl < -0.01:
                    hold_penalty = 0.003 * min(self.hold_steps, 20) / 20
                    reward -= hold_penalty
        ...
        if self.position == 0 and self.cooldown == 0:
            if trend == 1 and adx_gate:
                reward -= 0.001  # 상승 추세인데 관망

        # 하락장 관망 보상 (롱 전용이므로 하락장 관망이 올바른 행동)
        if self.position == 0 and trend == -1:
            reward += 0.001
        ...
        if self.balance < self.peak_balance * (1 - self.max_drawdown):
            truncated = True
            reward -= 1.0
