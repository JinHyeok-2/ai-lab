#!/usr/bin/env python3
# ETH 선물거래 RL 환경 — 부분 익절 (Partial Take Profit)
# exp02 기반 + 자동 50% 부분 익절
#
# 핵심 변경:
#   1. 포지션 사이즈 추적 (1.0 → 0.5 → 0.0)
#   2. 수익률 1.5% 이상 시 자동 50% 부분 익절 (첫 TP)
#   3. 부분 익절 후 나머지 50%는 에이전트가 청산 결정
#   4. 부분 익절 시 보상 지급 + 트레일링 유인
...
class ETHTradingEnvPartialTP(gym.Env):
    """부분 익절 환경: 50% 중간 익절 + 나머지 에이전트 결정"""

    metadata = {"render_modes": ["human"]}

    # 첫 TP 수익률 (레버리지 전 기준)
    PARTIAL_TP_PCT = 0.015   # 1.5% (3x 레버리지 시 ~4.5% 수익)
    PARTIAL_RATIO  = 0.5     # 50% 부분 청산
    ...
    def __init__(self, df: pd.DataFrame,
        ...
        # +5 상태: position, upnl, hold_steps, cooldown, pos_size
        n_features = len(self.feature_cols) + 5
    ...
    def _get_trend(self):
        ...
        if float(row.get("adx_norm", 0)) > 0.25:
            score *= 1.5
    ...
    def reset(self, seed=None, options=None):
        ...
        self.position      = 0      # 방향: 1(롱), -1(숏), 0(없음)
        self.pos_size      = 0.0    # 포지션 사이즈: 1.0 or 0.5
    ...
    def _get_obs(self):
        ...
        for i in range(self.current_step - self.window_size, self.current_step):
            ...
            feat += [
                ...
                float(self.pos_size),  # 포지션 사이즈 (0, 0.5, 1.0)
            ]
            rows.append(feat)
        return np.array(rows, dtype=np.float32).flatten()

    def _partial_close(self, price):
        """50% 부분 익절 실행"""
        pnl_pct = (price - self.entry_price) / self.entry_price * self.position
        fee = self.fee_rate
        # 50%만 청산
        partial_pnl = pnl_pct * self.leverage * self.PARTIAL_RATIO - fee
        self.balance += self.balance * partial_pnl * self.pos_size
        self.pos_size = self.pos_size * (1 - self.PARTIAL_RATIO)  # 1.0 → 0.5
        self.partial_done = True
        self.total_pnl += partial_pnl * self.PARTIAL_RATIO
        return partial_pnl
    ...
    def step(self, action: int):
        ...
        # ── 자동 부분 익절 체크 ─────────────────────────
        if self.position != 0 and not self.partial_done and self.pos_size == 1.0:
            pnl_pct = (price - self.entry_price) / self.entry_price * self.position
            if pnl_pct >= self.PARTIAL_TP_PCT:
                partial_pnl = self._partial_close(price)
                reward += partial_pnl * 10  # 부분 익절 보상
                reward += 0.02  # 익절 보너스
        ...
        def close_pos():
            ...
            fee = self.fee_rate
            # 남은 포지션 사이즈만큼 청산
            trade_pnl = pnl_pct * self.leverage * self.pos_size - fee
            ...
            self.trade_history.append({
                ...
                "position": self.position, "partial": self.partial_done,
            })
            self.position = 0
            self.pos_size = 0.0
            self.entry_price = 0.0
        ...
        if action == 1:   # 롱
            ...
            if self.position != 0:
                ...
                if self.position * trend > 0:
                    ...
                    reward -= 0.003
                # 부분 익절 후 남은 포지션 홀딩 보너스
                if self.partial_done and self.position * trend > 0:
                    reward += 0.003  # 추가 홀딩 유인
    ...
    def render(self):
        ...
        sz = f"({self.pos_size:.0%})" if self.position != 0 else ""
        print(f"Step {self.current_step:5d} | ${price:,.2f} | "
              f"잔고 ${self.balance:,.0f} | {pos_str}{sz} | "
