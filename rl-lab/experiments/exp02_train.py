#!/usr/bin/env python3
# exp02: 보상 대칭화 + 과다매매 억제 + DD 패널티 + ADX 필터
# 실행: conda run -n ai-lab python rl-lab/experiments/exp02_train.py
#
# exp01 대비 변경:
...
def train():
    ...
    meta = {
        "experiment": EXP_NAME,
        "description": "대칭 보상 + 과다매매 억제 + DD 패널티 + ADX 필터",
        "changes": [
...
def backtest(test_df):
    log.info("\n" + "=" * 60)
    log.info("  백테스트: exp02 vs v5 (베이스라인)")
    log.info("=" * 60)
    ...
    if exp_path.exists():
        log.info("exp02 백테스트...")
        env_exp = ETHTradingEnvV51Exp02(
            test_df, initial_balance=10000.0, leverage=LEVERAGE,
            window_size=20, min_hold_steps=4,
            max_episode_len=len(test_df) + 100,
            max_drawdown=1.0, cooldown_steps=8, curriculum=False,
        )
        results["exp02"] = _run_backtest(env_exp, exp_path)
...
def _plot_comparison(test_df, results, bh_ret):
    ...
    fig.suptitle(f"exp02 (symmetric + DD penalty + ADX filter) vs v5 [{INTERVAL}]",
                 fontsize=14, fontweight="bold")

    colors = {"v5": "#1565C0", "exp02": "#E65100"}
