#!/usr/bin/env python3
# 부분 익절 환경 + gamma 0.975 학습
# 50% 중간 익절(1.5%) + 나머지 에이전트 결정
...
from env_v51_partial_tp import ETHTradingEnvPartialTP
from env_v51_exp02 import ETHTradingEnvV51Exp02
from rl.env_v5 import ETHTradingEnvV5

EXP_NAME = "partial_tp"; INTERVAL = "30m"; LEVERAGE = 3; TRAIN_END = "2025-09-01"
...
class CB(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        log.info(f"[{self.start_time.strftime('%H:%M:%S')}] {EXP_NAME} 학습 시작 (gamma={GAMMA}, 부분익절)")
    def _on_step(self):
...
def train():
    ...
    def make_env(seed):
        def _init():
            env=ETHTradingEnvPartialTP(train_df, initial_balance=10000.0, leverage=LEVERAGE,
    ...
    with open(MODEL_DIR/"meta.json","w") as f:
        json.dump({"experiment":EXP_NAME,"gamma":GAMMA,
                   "description":"부분익절: 1.5% 도달시 50% 자동 청산, 나머지 에이전트 결정"},f,indent=2,ensure_ascii=False)
    log.info(f"\n모델 저장: {mp}.zip"); return model,test_df

def backtest(test_df):
    log.info("\n"+"="*60); log.info(f"  백테스트: {EXP_NAME} vs exp14 vs exp08"); log.info("="*60)
    bh=(test_df["close"].iloc[-1]-test_df["close"].iloc[20])/test_df["close"].iloc[20]*100
    results={}

    # exp14 (현재 최고 단일 모델)
    e14p=EXP_DIR/"models"/"exp14"/f"ppo_eth_{INTERVAL}.zip"
    if e14p.exists():
        e14=ETHTradingEnvV51Exp02(test_df,initial_balance=10000.0,leverage=LEVERAGE,window_size=20,
            min_hold_steps=4,max_episode_len=len(test_df)+100,max_drawdown=1.0,cooldown_steps=8,curriculum=False)
        results["exp14"]=_bt(e14,e14p)
    ...
    # partial_tp (부분 익절 환경에서 테스트)
    ep=MODEL_DIR/f"ppo_eth_{INTERVAL}.zip"
    if ep.exists():
        ee=ETHTradingEnvPartialTP(test_df,initial_balance=10000.0,leverage=LEVERAGE,window_size=20,
            min_hold_steps=4,max_episode_len=len(test_df)+100,max_drawdown=1.0,cooldown_steps=8,curriculum=False)
        results[EXP_NAME]=_bt_partial(ee,ep)
    ...
    for n,r in results.items():
        ...
        if "partial_closes" in s:
            log.info(f"  부분 익절: {s['partial_closes']}회")
...
def _bt_partial(env,mp):
    """부분 익절 환경 백테스트"""
    model=PPO.load(str(mp),device="cpu"); obs,_=env.reset()
    bal=[env.initial_balance]; act=[]; done=False; partial_count=0
    prev_pos_size = 0.0
    while not done:
        a,_=model.predict(obs,deterministic=True); obs,_,te,tr,info=env.step(int(a))
        done=te or tr; bal.append(info["balance"]); act.append(int(a))
        # 부분 익절 카운트
        if info.get("pos_size", 0) == 0.5 and prev_pos_size == 1.0:
            partial_count += 1
        prev_pos_size = info.get("pos_size", 0)
    arr=np.array(bal); pk=np.maximum.accumulate(arr); mdd=((arr-pk)/pk*100).min()
    return {"balances":bal,"actions":act,"stats":{"final":bal[-1],"return":(bal[-1]-10000)/10000*100,
            "mdd":mdd,"trades":info["total_trades"],"win_rate":info["win_rate"],
...
def _plot(test_df,results,bh):
    ...
    fig.suptitle(f"{EXP_NAME} (partial TP) vs exp14 vs exp08",fontsize=13,fontweight="bold")
    colors={"exp14":"#E65100","exp08":"#1565C0",EXP_NAME:"#2E7D32"}
    for n,r in results.items():
