#!/usr/bin/env python3
# exp08: gamma 단축 (0.995 → 0.99, exp02 환경 동일)
# 단기 보상 우선 → 더 빈번한 거래 유도
...
class CurriculumCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        log.info(f"[{self.start_time.strftime('%H:%M:%S')}] {EXP_NAME} 학습 시작 ({self.total_steps:,} 스텝, gamma=0.99)")
...
def train():
    ...
    meta = {
        "experiment": EXP_NAME,
        "description": "gamma 단축 (0.995 → 0.99, exp02 환경 동일)",
        "changes": ["gamma: 0.995 → 0.99 (단기 보상 우선)"],
...
def _plot(test_df, results, bh_ret):
    ...
    fig.suptitle(f"{EXP_NAME} (gamma=0.99) vs exp02 vs v5", fontsize=13, fontweight="bold")
