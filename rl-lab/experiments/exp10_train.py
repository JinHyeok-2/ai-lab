#!/usr/bin/env python3
# exp10: exp08(gamma 0.99) + DD 패널티 강화 (MDD -20.8% → -15% 목표)
# DD 3단계: 3%/-0.03, 5%/-0.08, 10%/-0.20, 12%/truncate
...
from env_v51_exp10 import ETHTradingEnvV51Exp10
from env_v51_exp02 import ETHTradingEnvV51Exp02
from rl.env_v5 import ETHTradingEnvV5

EXP_NAME = "exp10"; INTERVAL = "30m"; LEVERAGE = 3; TRAIN_END = "2025-09-01"
N_ENVS = 4; TOTAL_STEPS = 3_000_000
...
def train():
    ...
    def make_env(seed):
        def _init():
            env = ETHTradingEnvV51Exp10(train_df, initial_balance=10000.0, leverage=LEVERAGE,
                fee_rate=0.0004, window_size=20, min_hold_steps=4,
                max_episode_len=2000, max_drawdown=0.12, cooldown_steps=8, curriculum=True)
    ...
    meta = {"experiment": EXP_NAME, "description": "exp08 + DD 패널티 강화 (3%/-0.03, 5%/-0.08, 10%/-0.20, 12%/truncate)",
            "changes": ["DD 3단계 강화", "max_drawdown: 15% → 12%", "gamma: 0.99"]}
...
def backtest(test_df):
    log.info("\n" + "=" * 60); log.info(f"  백테스트: {EXP_NAME} vs exp08 vs v5"); log.info("=" * 60)
    ...
    if exp08_path.exists():
        ...
        results["exp08"] = _run_backtest(env08, exp08_path)
    exp_path = MODEL_DIR / f"ppo_eth_{INTERVAL}.zip"
    if exp_path.exists():
        env10 = ETHTradingEnvV51Exp10(test_df, initial_balance=10000.0, leverage=LEVERAGE,
            window_size=20, min_hold_steps=4, max_episode_len=len(test_df)+100, max_drawdown=1.0, cooldown_steps=8, curriculum=False)
        results[EXP_NAME] = _run_backtest(env10, exp_path)
...
def _plot(test_df, results, bh_ret):
    ...
    fig.suptitle(f"{EXP_NAME} (DD penalty++) vs exp08 vs v5", fontsize=13, fontweight="bold")
