#!/usr/bin/env python3
# ETH 선물거래 RL 환경 v5.1-exp10
# exp08(gamma 0.99) 기반 + DD 패널티 강화 (MDD -20.8% → -15% 목표)
#
# exp02 대비 변경:
#   1. DD 패널티 강화: 5%→-0.08, 10%→-0.20, 12%→truncate (exp02: 5%/-0.05, 10%/-0.15, 15%/truncate)
#   2. 나머지 exp02 동일 (cooldown 8, ADX 필터 등)
...
class ETHTradingEnvV51Exp10(gym.Env):
    """v5.1-exp10: exp08 기반 + DD 패널티 강화"""
    ...
    def __init__(self, df: pd.DataFrame,
                 ...
                 max_drawdown: float = 0.12,    # 15% → 12% (더 엄격)
    ...
    def step(self, action: int):
        ...
        if action == 1:
            ...
            if self.position == 0:
                if self.cooldown > 0: reward -= 0.008 * self.cooldown
                elif adx_gate:
        ...
        if drawdown > 0.10:
            reward -= 0.20   # DD 10% 이상: 매우 강한 패널티 (exp02: -0.15)
        elif drawdown > 0.05:
            reward -= 0.08   # DD 5% 이상: 강화 (exp02: -0.05)
        elif drawdown > 0.03:
            reward -= 0.03   # DD 3% 이상: 새로 추가 — 조기 경고
        ...
        truncated = (self.balance < self.initial_balance * 0.05
                     or drawdown > self.max_drawdown)  # 12%에서 truncate
