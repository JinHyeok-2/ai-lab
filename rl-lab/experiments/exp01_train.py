#!/usr/bin/env python3
# exp01: 보상 대칭화 실험 — 학습 + 백테스트 + v5 비교
# 실행: conda run -n ai-lab python rl-lab/experiments/exp01_train.py
#
# v5 대비 변경:
#   1. 추세 순행/역행 보상 대칭화 (+0.012 / -0.012)
#   2. 커리큘럼 하락장 초기 비율 50% (v5: 70%)
...
from env_v51 import ETHTradingEnvV51
...
def train():
    ...
    meta = {
        "experiment": EXP_NAME,
        "description": "보상 대칭화 (숏 편향 해소)",
        "changes": [
            "추세 순행/역행 보상 대칭: +0.012/-0.012 (v5: 숏+0.02/롱+0.008)",
            "커리큘럼 하락장 비율: 50%→30%→자연 (v5: 70%→40%→자연)",
            "관망 보상 대칭화: position*trend 기반",
        ],
...
def backtest(test_df):
    log.info("\n" + "=" * 60)
    log.info("  백테스트: exp01 (v5.1) vs v5 (베이스라인)")
    log.info("=" * 60)
    ...
    if exp_path.exists():
        log.info("exp01 (v5.1) 백테스트...")
        env_exp = ETHTradingEnvV51(
            test_df, initial_balance=10000.0, leverage=LEVERAGE,
            window_size=20, min_hold_steps=4,
            max_episode_len=len(test_df) + 100,
            max_drawdown=1.0, curriculum=False,
        )
        results["exp01"] = _run_backtest(env_exp, exp_path)
...
def _plot_comparison(test_df, results, bh_ret):
    ...
    fig.suptitle(f"exp01 (reward symmetric) vs v5 baseline [{INTERVAL}]",
                 fontsize=14, fontweight="bold")

    colors = {"v5": "#1565C0", "exp01": "#E65100"}
