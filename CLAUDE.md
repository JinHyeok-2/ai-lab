...
코인 분석 요청 시 **반드시 롱/숏 양방향 비교** 형식으로 보고:

### 롱 진입 체크리스트 (8항목)
1. EMA20 > EMA50 (정배열)
2. +DI > -DI (상승 지배)
3. MACD hist > 0 (상승 모멘텀)
4. 펀딩비 < -0.1% (숏 과밀 → 스퀴즈)
5. RSI 15m < 70 (과매수 아님)
6. 가격 > EMA20 (추세 위)
7. BB 중간선 위 (강세)
...
### 판단 기준
- 양쪽 점수 비교 → 높은 쪽이 유리 방향
- 점수 차이 3+ → "강한 시그널"
- 점수 차이 1~2 → "약한 시그널, 관망 권장"
- 동점 → "중립, 진입 자제"

### 스캘핑 시간별 진입 가이드 (수동 단타용)

사용자가 단기 스캘핑 요청 시 **목표 보유시간별 진입 전략** 제시:

**0~5분 (초단타)** — 건당 $0.61 평균, 승률 50%
- 타임프레임: **1m + 5m** 캔들
- 진입: 15m RSI 극단(20 이하 or 80 이상)에서 **1m 반전 캔들** 확인 후 진입
- TP: ATR(5m) × 1.0 (~0.5~1%)
- SL: ATR(5m) × 0.5 (타이트, 즉시 손절)
- 적합: 급등/급락 직후 반등, 거래량 스파이크
- 주의: 수수료 비중 높음 (0.08% 왕복) → 최소 0.3%+ 이동 필요

**5~30분 (단타)** — 건당 평균 마이너스 ⚠️ (현재 봇 데이터 기준)
- 타임프레임: **5m + 15m**
- 진입: 15m BB 하단/상단 + RSI 과매도/과매수 + 5m MACD 전환
- TP: BB 중간선 (15m)
- SL: BB 외곽 + 0.2%
- 적합: 횡보장, ADX < 20
- 주의: **이 구간이 실측 최악** (건당 -$0.15~-$0.64) → 확실한 시그널만

**30분~2시간 (스윙 단타)** — 가장 안정적 구간 ★
- 타임프레임: **15m + 1h**
- 진입: 1h BB 위치 + RSI + 추세 방향 확인 후 15m 눌림목에서 진입
- TP: 1h BB 반대편 or ATR × 2.0
- SL: 1h EMA20 이탈 (롱) / EMA20 돌파 (숏)
- 적합: CVD 패턴, 모멘텀 추세 합류
- 이 구간 승률 **59%**, 건당 **+$0.15** (최적)

**시간별 요약표 (반드시 표시)**
| 구간 | 승률 | 건당PnL | 권장 | 위험도 |
|------|------|---------|------|--------|
| 0~5분 | 50% | +$0.27 | 급등/급락 직후만 | 높음 |
| 5~30분 | 52% | -$0.36 | 비추 (실측 마이너스) | 매우높음 |
| 30분~2h | 51% | +$0.02 | **권장** (안정적) | 중간 |
| 2h+ | 42% | -$0.07 | 봇에 맡기기 | 낮음 |

## 작업 규칙

- compact 후 체크리스트 진행 상황 확인 → 다음 작업 추천
- `.env` 파일은 읽기만, 수정 시 사용자 확인 필수
- **플랜모드 완료 후 자동 구현**: ExitPlanMode 승인 시 바로 코드 작성 + 구문 검사 + 봇 재시작 + 커밋까지 자동 진행 (사용자 추가 확인 불필요)
- **코드 수정 후 5회 점검 필수**: position_updater.py 수정 시 반드시 구문검사 + 핵심 체크(BLACKLIST/하드코딩/활성전략/안전장치) 5회 반복 후 결과 보고. 문제 발견 시 수정 → 재점검 반복.
