...
#   python trading/rl/backtest.py --interval 30m --version v3 --split 0.6
#   python trading/rl/backtest.py --interval 30m --version v4 --test-start 2025-09-01
...
def run_backtest(interval: str = "1h", version: str = "v2",
    ...
    if test_start:
        ...
        print(f"메타데이터 기준 분할 (학습 시 설정 그대로)")
    else:
    ...
    print(f"총 {len(test_df)}개 캔들\n")

    # 환경 및 모델 로드
    from train import MIN_HOLD
    env   = ETHTradingEnv(test_df, initial_balance=10000.0, leverage=5,
                          window_size=20, min_hold_steps=MIN_HOLD.get(interval, 3))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--version",    default="v2", help="모델 버전 (예: v1, v2)")
    parser.add_argument("--split",      type=float,   default=None,
                        help="테스트 시작 비율 (예: 0.6 → 60%% 이후 테스트)")
    parser.add_argument("--test-start", default=None,
                        help="테스트 시작 날짜 (예: 2025-09-01) — 지정 시 --split 무시")
